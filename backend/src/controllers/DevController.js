...
module.exports = {
  ...
  async store(request, response) {
    ...
    if (!dev) {
      try {
        ...
        return response
          .status(400)
          .json({ error: 'Something did not work as expected.' });
      }
    } else {
      return response
        .status(400)
  ...
  async update(request, response) {
    ...
    const devUpdate = await Dev.findById(request.params.id);
    ...
    if (devUpdate) {
      try {
        ...
        const { name = login, avatar_url, bio, id } = apiResponse.data;
        ...
        const devUpdate = await Dev.updateOne({
          id,
          github_username,
          name,
          techs: techsArray,
        ...
        sendMessage(sendSocketMessageTo, 'new-dev', devUpdate);
